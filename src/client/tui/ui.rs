...
use crate::client::app::ClosedDatabaseAppState;
use crate::client::app::DatabaseState;
...
fn render_active_table(f: &mut Frame, layout: Rect, color: Color, table_result: Result<crate::core::table::Table, String>, selected_row: usize, selected_column: usize) {
        ...
        for (cell_number, cell) in row.get_values().iter().enumerate() {
            let cell_content =
                match cell.get_value() {
                    crate::core::types::ValueType::Int(int) => {
                        int.get_value().to_string()
                    },
                    crate::core::types::ValueType::Str(str) => {
                        str.get_value().to_owned()
                    },
                    crate::core::types::ValueType::Real(real) => {
                        real.get_value().to_string()
                    },
                    crate::core::types::ValueType::Pic(_picture) => {
                        "picture".to_owned()
                    },
                    crate::core::types::ValueType::Char(char) => {
                        char.get_value().to_string()
                    },
                    crate::core::types::ValueType::Date(date) => {
                        date.get_value().to_string()
                    },
                    crate::core::types::ValueType::Email(email) => {
...
fn render_main_screen(f: &mut Frame, app: &mut App) {
    if let DatabaseState::Opened(state) = app.get_database_state() {
        ...
        match state {
            crate::client::app::OpenedDatabaseAppState::ActiveHood(e) => {
            ...
            crate::client::app::OpenedDatabaseAppState::ActiveMenu => {
            ...
            crate::client::app::OpenedDatabaseAppState::ActiveTable => {
                render_active_menu(f, layout[0], Color::White, app.get_database_name(), app.get_table_list(), app.get_selected_table_index());
                render_screen_hood(f, inner_layout[0], Color::White, "".to_owned());
            ...
            crate::client::app::OpenedDatabaseAppState::None => {
                render_active_menu(f, layout[0], Color::White, app.get_database_name(), app.get_table_list(), app.get_selected_table_index());
                render_screen_hood(f, inner_layout[0], Color::White, "".to_owned());
                render_active_table(f, inner_layout[1], Color::White, app.get_current_table(), app.get_selected_row_index(), app.get_selected_column_index());
            },
